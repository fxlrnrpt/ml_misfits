...
def find_best_split(feature_vector, target_vector):
    ...
    * За наличие в функции циклов балл будет снижен. Векторизуйте! :)
    ...
    gini_best_sub_idx = np.argmax(ginis)

    return thresholds_value, ginis, thresholds_value[gini_best_sub_idx], ginis[gini_best_sub_idx]
...
class DecisionTree:
    ...
    def _fit_node(self, sub_X, sub_y, node):
        ...
        for feature in range(sub_X.shape[1]):
            feature_type = self._feature_types.get(feature, "real")
            categories_map = {}
            ...
            if gini_best is None or gini > gini_best:
                ...
                if feature_type == "real":
                    ...
                    threshold_best = list(
                        map(lambda x: x[0], filter(lambda x: x[1] < threshold, categories_map.items()))
                    )
                else:
                    raise ValueError
        ...
        feature_best_type = self._feature_types.get(feature_best, "real")
        if feature_best_type == "real":
            node["threshold"] = threshold_best
        elif feature_best_type == "categorical":
...
class LinearRegressionTree:
    def __init__(
        self, feature_types, base_model_type=None, max_depth=None, min_samples_split=None, min_samples_leaf=None
    ):
        pass
